...
| 3 | Soft music, off by default | ✔️ | 🟢 | **Done:** optional ambient loop in the 3D gallery, exposed as the `Ambient music` switch in the hamburger menu's Preferences section | Never autoplay; it is not remembered across visits, so an old choice cannot start sound unexpectedly. Deliberately absent from the door and list view, which remain quiet |
| 4 | Slower, gentler motion | ✔️ | 🟢 | `prefers-reduced-motion` baseline + slower camera/chrome timing | Continue tuning by feel |
| 5 | Readable text | ✔️ | 🟢 | Baseline typography/contrast in current UI; list view uses real HTML text (fully readable/zoomable). **Done:** in-scene 3D plaque (`components/museum/Frame.tsx`) enlarged ~20% (canvas, font sizes, and the physical plaque mesh together, so it's an actual size increase, not just text squeezed into the same space); the bare `'...'` hint replaced with "Tap to read more" | Keep validating on real devices; revisit plaque layout/content only if bigger text still isn't enough |
...
| 20 | "Blind" fully-audio experience | 🔴 | 🟢 | Deliberately not built | The accessible HTML list (#30) is the primary blind/screen-reader route: it exposes each artwork's editable visual alt text, written context, and its optional narration with ordinary, user-controlled navigation. We decided not to add a separate auto-advancing SR-only audio player: browsers cannot reliably wait for a screen reader to finish speaking before playing the next audio, and it would reduce the visitor's control. Revisit only after a real screen-reader audit identifies a gap |
...
| 24 | "Add to my shelf" — private favourites | ✔️ | 🟡 | **Done:** `ShelfContext`, heart button on plaque, "My shelf" section in hamburger menu, stable IDs (incl. static placeholders) | Device-local only. My Shelf hides Rooms/Preferences/Controls while open; it does not show content notes |
